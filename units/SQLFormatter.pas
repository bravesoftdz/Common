...
begin
  Create(CHR_TAB, 4, 999, False, False, False, True, True, True, False, True, False);
end;
...
begin
  ...
  if ContentElement.NodeName = '#text' then
    Exit;
  ...
  begin
    ...
    begin
      ...
      if FExpandCommaLists and
        not (Assigned(ContentElement.ParentNode) and ((ContentElement.ParentNode.NodeName = SQLConstants.ENAME_DDLDETAIL_PARENS) or
  ...
  begin
    WhiteSpaceSeparateWords(State);
    State.SetRecentKeyword(ContentElement.Attributes[SQLConstants.ANAME_SIMPLETEXT]);
    State.AddOutputContent(FormatKeyword(contentElement.Attributes[SQLConstants.ANAME_SIMPLETEXT]));
