...
type
  TSQLTokenType = (ttOpenParens, ttCloseParens, ttWhiteSpace, ttOtherNode, ttSingleLineComment,
...
begin
  Result := FSQLTokenList.Items[FCurrentTokenID].Value;
end;
...
begin
  ...
  Result := IsWhitespace(CurrentCharacter) or IsOperatorCharacter(CurrentCharacter) or
    (IsCurrencyPrefix(CurrentCharacter) and (CurrentCharacter <> '$')) or
