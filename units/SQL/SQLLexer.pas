{ Based on David J Butlers SQL lexical parser }
  ...
  if Result = ttEND then
    if FReader.Peek(B[0], 5) = 5 then
      if (B[0] = '-') and (UpCase(B[1]) = 'E') and (UpCase(B[2]) = 'X') and
         (UpCase(B[3]) = 'E') and (UpCase(B[4]) = 'C') then
        begin
          FReader.Skip(5);
  ...
  case FTokenType of
    ...
    begin
      ...
      if TokenIsKeyword then
        ...
        R := '<#' + AnsiString(IntToStr(FTokenType)) + '>';
    end;
  end;
  if FTokenType in [
